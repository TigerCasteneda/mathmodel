...
class FakeZenodoNode:
    """Per-node stub mimicking Scrapling's element wrapper.

    Carries a backing lxml element so sub-selectors like `h4 a` filter
    descendants, mirroring Scrapling's real behaviour.
    """
    ...
    def css(self, selector):
        if self._lxml is not None:
            try:
                ...
                return FakeZenodoCssChain(nodes)
            except Exception:
                pass
        return FakeZenodoCssChain(self._children)


class FakeZenodoCssChain:
    """Chain with `.first`, `.css(...)`, and iterable semantics matching Scrapling."""
    ...
    def css(self, selector):
        # Recursively apply selector against each node's backing lxml element.
        nodes: list[FakeZenodoNode] = []
        for node in self._nodes:
            lxml_el = getattr(node, "_lxml", None)
            if lxml_el is None:
                continue
...
def _build_zenodo_fake_scrapling(monkeypatch, html):
    """Inject a Selector stub that parses Zenodo result-item blocks."""
    from lxml import html as lxml_html

    tree = lxml_html.fromstring(html)
    blocks = tree.cssselect("div.result-item")
    if not blocks:
        blocks = tree.cssselect("article.result")

    # Wrap each block in a FakeZenodoNode that carries its lxml element so
    # production code's ``node.css(...)`` recurses via FakeZenodoCssChain.
    wrapper_nodes = [FakeZenodoNode(lxml_element=block) for block in blocks]

    by_selector = {
        "div.result-item": wrapper_nodes,
        "article.result": wrapper_nodes,
    }
    for block in blocks:
        title_a = block.cssselect("h4 a") or block.cssselect("a.result-title")
        if title_a:
            text = title_a[0].text_content().strip() if title_a[0].text else ""
            href = title_a[0].get("href", "")
            for selector in ("h4 a", "a.result-title"):
                by_selector.setdefault(selector, []).append(
                    FakeZenodoNode(text=text, attrib={"href": href})
                )
        authors = block.cssselect("span.result-authors")
        if authors:
            by_selector.setdefault("span.result-authors", []).append(
                FakeZenodoNode(text=authors[0].text_content())
            )
        desc = block.cssselect("p.result-description")
        if desc:
            by_selector.setdefault("p.result-description", []).append(
                FakeZenodoNode(text=desc[0].text_content())
            )
        date = block.cssselect("span.result-date")
        if date:
            by_selector.setdefault("span.result-date", []).append(
                FakeZenodoNode(text=date[0].text_content())
            )
    ...
    def _selector_factory(content=None, url=None):
        ...
        def _css(selector):
            nodes = by_selector.get(selector, [])
            return FakeZenodoCssChain(nodes)
...
@pytest.mark.asyncio
async def test_search_zenodo_html_parses_result_blocks(monkeypatch):
    """search_zenodo_html should return Zenodo-shaped items."""
    from app.providers.scrapling_search import search_zenodo_html

    html = """
    <html><body>
    <div class="result-item">
        <h4><a href="/record/12345">Test Dataset</a></h4>
        <span class="result-authors">Doe, J.; Smith, K.</span>
        <p class="result-description">A comprehensive dataset for testing...</p>
        <span class="result-date">January 1, 2024</span>
    </div>
    </body></html>
    """
    fetcher_cls = _build_zenodo_fake_scrapling(monkeypatch, html)
    fake_page = FakePage(html)
    fetcher_cls.get.return_value = fake_page

    items = await search_zenodo_html("test", limit=5)
    assert len(items) == 1
    item = items[0]
    assert item["title"] == "Test Dataset"
    assert item["url"] == "https://zenodo.org/record/12345"
    assert item["provider"] == "scrapling_zenodo"
    assert item["source"] == "scrapling_zenodo_search"
    assert item["category"] == "dataset"
    assert item["publish_year"] == 2024
    assert "Doe" in (item["authors"] or "")


@pytest.mark.asyncio
async def test_search_zenodo_html_returns_empty_on_failure(fake_scrapling):
    """Network failure should return empty list, not raise."""
    from app.providers.scrapling_search import search_zenodo_html
...
@pytest.mark.asyncio
async def test_search_zenodo_html_uses_basic_fetcher(fake_scrapling):
    """Zenodo has no anti-bot - must use Fetcher, not StealthyFetcher."""
    from app.providers.scrapling_search import search_zenodo_html

    fake_page = FakePage("<html></html>")
    fake_scrapling["fetcher_cls"].get.return_value = fake_page

    await search_zenodo_html("test", limit=3)

    assert fake_scrapling["fetcher_cls"].get.called, "Fetcher.get was not called"
    assert not fake_scrapling["stealthy_cls"].fetch.called, "StealthyFetcher was used (wrong for Zenodo)"


@pytest.mark.asyncio
async def test_search_zenodo_html_skips_blocks_without_url(monkeypatch):
    """Blocks missing a URL or title are skipped, not erroring the whole result."""
    from app.providers.scrapling_search import search_zenodo_html
    ...
    <html><body>
    <div class="result-item">
        <span class="result-authors">NoTitleAuthor</span>
    </div>
    <div class="result-item">
        <h4><a href="/record/99999">Real Dataset</a></h4>
        <span class="result-authors">Doe J</span>
        <span class="result-date">March 5, 2023</span>
    </div>
    </body></html>
    """
    fetcher_cls = _build_zenodo_fake_scrapling(monkeypatch, html)
    ...
    items = await search_zenodo_html("test", limit=5)
    assert len(items) == 1
    assert items[0]["url"] == "https://zenodo.org/record/99999"
    assert items[0]["publish_year"] == 2023
